...
;-
  ...
  for j = 0, nds-1 do begin
     ...
     if (ity ge 0) then begin   ; Ordinary X-Y data
        xyvals = dblarr(ity+2, ndata > 2)
        readf, ilu, xyvals
        if ndata eq 1 then xyvals = xyvals[*, 0]
        xydata = {graff_xydata}
        nerrv = gr_n_errors(ity)
        xydata.x = ptr_new(reform(xyvals[0, *]))
        xydata.y = ptr_new(reform(xyvals[1, *]))
        if nerrv[0] ne 0 then  $
           xydata.x_err = ptr_new(xyvals[2:1+nerrv[0], *])
        if nerrv[1] ne 0 then $
           xydata.y_err = ptr_new(xyvals[2+nerrv[0]:*, *])
