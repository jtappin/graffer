...
;
; History:
;	Original (after gr_fcopy_menu): 7/2/12; SJT
;-

function xycopy_event, event
...
case but of
    ...
    "ALL": begin
        ...
        if event.select then $
          locs = where(data.type ge 0 and data.type ne 9) $
        else locs = where(data.type eq data[uvs.current].type)
        ll = where(locs ne uvs.current)
        locs = locs[ll]
        tc = ['XY    ', 'XYE   ', 'XYEE  ', 'XYF   ', 'XYFF  ', 'XYFE  ', $
              'XYFEE ', 'XYFFE ', 'XYFFEE']
        types = tc[data[locs].type]

        idx = string(locs+1, format = "(I3,'] ')")
        ndata = string(data[locs].ndata, format = "('(',I5,')')")

        desc = idx+types+': '+ndata+' '+data[locs].descript
...
function gr_xycopy_menu, pdefs

data = *pdefs.data

funlocs = where(data.type ge 0 and data.type ne 9 and data.ndata gt 0, nf)

if nf eq 0 || nf eq 1 && funlocs[0] eq pdefs.cset then begin
    junk = dialog_message(['There are no XY datasets', $
                           'available to copy'], $
                          /info, $
...
if (data[pdefs.cset].type eq 9 || $
    data[pdefs.cset].type lt 0) then begin
    ans = dialog_message(['Current dataset is not an XY dataset.', $
...
if data[pdefs.cset].ndata gt 0 && $
  data[pdefs.cset].type ge 0 && $
  data[pdefs.cset].type ge 0 ne 9 then begin
    locs = where(data.type eq data[pdefs.cset].type and $
                 data.ndata gt 0, nmf)
    ...
    if nmf eq 0 || nnc eq 0 then begin
        ans = dialog_message(['There are no datasets of the same', $
                              'kind as the current dataset', $
                              'Do you want to use all datasets?'], $
                             /question, $
                             dialog_parent = pdefs.ids.graffer, $
                             resource = 'Graffer')
        if ans eq 'No' then return, 0
...
return, gr_xy_copy(pdefs, idx, force = uvs.all)
