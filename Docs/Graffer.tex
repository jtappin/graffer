...
(there are plenty of those around e.g. \texttt{xfig}, \texttt{inkscape}
or commercial products) but rather tries to do the things that they
(well \texttt{xfig} at any rate) don't do well i.e.\ plotting data.
...
The IDL/GDL version requires IDL or a recent GIT snapshot of GDL
(\url{https://github.com/gnudatalanguage/gdl}, at the time of writing a
number of key features needed were not in the latest release version).

\subsection{Copyright etc.}

The Fortran parts of GRAFFER may be freely
used, copied \& distributed under the terms of the GNU General Public
Licence V3.

The IDL/GDL parts may be freely
used, copied \& distributed under the terms of the GNU General Public
Licence V2 or any later version.
...
If you are only installing the IDL/GDL part, then the paths must be
configured manually.

\subsection{Invoking it (Fortran)}
...
\item [\texttt{-{}-geometry} or \texttt{-g}:] Specify a geometry for the
  ...
  \texttt{-{}-geometry=800x800}). The default is \texttt{800x800}.
...
\item[\texttt{-{}-gdl} or \texttt{-{}-idl}:]
  ...
  (e.g. \texttt{--gdl /usr/local/bin/gdl-git}). Default is to try first
  the \texttt{GRAFFER\_GDL} environment variable then to try first
  \texttt{gdl} and then \texttt{idl} in the user's path.
\end{description}

\subsection{Invoking it (IDL/GDL)}
\label{sec:invoke-idl}

If GRAFFER is in your \texttt{!path}, then you just need to type:
\begin{verbatim}
graffer
\end{verbatim}
or
...
graffer, 'file.grf'
\end{verbatim}
at the IDL or GDL prompt.

\subsubsection{Keywords}
\label{sec:idl-keywords}

\begin{description}
\item[\texttt{group}	\textit{long}]	The group leader of the widget tree.
\item[\texttt{xsize}	\textit{int}]	The x dimension of the draw widget
\item[\texttt{ysize}	\textit{int}]	The y dimension of the draw widget
\item[\texttt{/noscroll}]	If set do not make the draw window
scrolling even if it is bigger than
800 pixels.
\item[\texttt{/debug}]	If set, then run in debugging mode.
\item[\texttt{/block}]			If set, then run the widgets in
blocking mode
\item[\texttt{/recover}]	If set, then attempt to recover from
an autosave file.
\item[\texttt{/ttype}]	If set, then make a new file use
...
\item[\texttt{/bitmaps}]	If set, then the plot symbol and
...
\begin{verbatim}
  x * sin(y)
  -1./sqrt(((x+.5)^2+y^2)>1e-6) - .5/sqrt(((x-1)^2+y^2)>1e-6) - r^2
\end{verbatim}
\end{description}
...
Note also that \texttt{r} may be used and is automatically interpreted
as \verb|sqrt(x^2 + y^2)|.
...
\begin{description}
\item[\texttt{file} \textit{string}] The graffer file to modify.
\item[\texttt{idx} \textit{int}] The dataset index to modify
  (starting at 1).
\end{description}
...
\item[\texttt{style} \textit{int}] The standard IDL linestyle codes
\item[\texttt{psym} \textit{int}] The GRAFFER symbol code - extended IDL
  symbol
  codes.
\item[\texttt{join} \textit{int}] The style of joining: 0 - none
  1 - sloping lines
  2 - histogram
\item[\texttt{symsize} \textit{float}] The size for the symbols (relative to
  standard)
\item[\texttt{colour} \textit{int}] Colour number - standard GRAFFER
  colours (which may
  well not work on
...
\item[\texttt{/sort}] Whether to sort the values on the X
  axis.
\item[\texttt{/ascii}] If set, then save as an ASCII GRAFFER
  file (by default a binary graffer file
  is generated).
\item[\texttt{/noclip}] If  set, then disable clipping to the
  axis box. 
\item[\texttt{mouse} \textit{int}] If explicitly set to zero then
  disable mouse-editing of the dataset.
\item[\texttt{z\_format} \textit{int}] For 2-D datasets, select display
  format (0=contour, 1=colour image)
\item[\texttt{z\_nlevels} \textit{int}] For 2D datasets, select number of
  automatic contours
\item[\texttt{z\_lmap} \textit{int}] Select mapping for automatic contour
  levels. 0=linear, 1=log, 2=sqrt
\item[\texttt{z\_levels} \textit{float}] For 2D datasets, select levels for
  explicit contours
\item[\texttt{z\_colours} \textit{int}] For 2D datasets, select the colours
  for the contours
\item[\texttt{z\_style} \textit{int}] For 2D datasets, select linestyles for contours
\item[\texttt{z\_thick} \textit{float}] For 2D datasets, select line
  thicknesses for contours
\item[\texttt{z\_label} \textit{int}] Specify the interval of contours for labelling.
\item[\texttt{/z\_fill}]  input If set, then fill the contours.
\item[\texttt{z\_range} \textit{int}] For 2-D datasets, specify the cutoff
  range for image displays
\item[\texttt{z\_pxsize} \textit{float}] For 2D datasets, specify the pixel
  size to use in images for PS device.
\item[\texttt{/z\_invert}] image display, invert the colour
  table if set.
\item[\texttt{z\_mode} \textit{int}] Z image scaling mode, 0=linear, 1=log, 2=sqrt.
\item[\texttt{z\_ctable} \textit{int}] Select a colour table for 2-D display
  of images.
\item[\texttt{y\_axis} \textit{int}] Specify which Y axis to use. (0 or 1)
...
\item[\texttt{z\_func} \textit{string}] New f(x,y).
...
\item[\texttt{frange}  \textit{float}] The range of x, y or t over which to
  plot a function
\item[\texttt{z\_missing} \textit{float}] A missing value to use for warped images.
\item[\texttt{x\_scale} \textit{float}] Scale X values by this factor
\item[\texttt{x\_shift} \textit{float}] Shift X values by this factor
\item[\texttt{y\_scale} \textit{float}] Scale Y values by this factor
\item[\texttt{y\_shift} \textit{float}] Shift Y values by this factor
\item[\texttt{z\_scale} \textit{float}] Scale Z values by this factor
  (2-D datasets only).
\item[\texttt{z\_shift} \textit{float}] Shift Z values by this factor
  (2-D datasets only). 
\item[\texttt{status} \textit{int}] A named variable to be set to 0 on
  failure or 1 on success
\item[\texttt{/retain\_unset}] input If set, then unspecified data fields
...
The \texttt{/RETAIN\_UNSET} key allows single axes of a dataset to be
changed without resetting the others (like specifying a dot in the GUI
to read local variables).  To work, the change must not affect the
dimension of the dataset (for Z datasets a 1-D X or Y map can be
replaced by 2-D or vice-versa, provided Z is still matched).  If
specified, error limits must provide all errors.  If a dataset has
errors, but no new ones are given, then unchanged axes retain their
errors, while changed axes lose theirs.
